# Audit daté — Matrice wizard + qualification live (runtime)

> Type documentaire: `audit daté`
> Date de référence: `2026-03-04`
> Statut: snapshot historique d'un état runtime
> Doctrine: `Docs/_spec` reste la source de vérité canonique.

Portée: état des règles dans `site-factory` (normalisation wizard + qualification runtime) au 2026-03-04.

## 1) Combinaisons effectives (type runtime × hosting × famille technique)

> Règle appliquée: la famille technique finale autorisée = intersection entre:
> - familles techniques autorisées par le type runtime (`ProjectType`),
> - familles techniques autorisées par la cible d'hébergement.
>
...
Rappels runtime de départ:
- Catégorie initiale runtime: `BLOG/VITRINE = CAT0`, `ECOM/APP = CAT2`.
