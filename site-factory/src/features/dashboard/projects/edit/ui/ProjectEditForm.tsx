...
import { PROJECT_TYPE_LABELS } from "@/lib/validators";
import {
  getMandatoryModules,
  getIncludedModules,
  isModuleCompatible,
...
import { deriveOfferProjectType } from "@/lib/wizard-domain";
...
const PROJECT_TYPES = [
  { value: "STARTER", label: PROJECT_TYPE_LABELS.STARTER, desc: "Mini-site, petit budget" },
  { value: "VITRINE", label: PROJECT_TYPE_LABELS.VITRINE, desc: "Site de présentation classique" },
  { value: "BLOG", label: PROJECT_TYPE_LABELS.BLOG, desc: "Blog ou magazine en ligne" },
  { value: "ECOM", label: PROJECT_TYPE_LABELS.ECOM, desc: "Boutique en ligne" },
  { value: "APP", label: PROJECT_TYPE_LABELS.APP, desc: "Application web avancée" },
] as const;
...
export function ProjectEditForm({ project }: ProjectEditFormProps) {
  ...
  const normalizedProjectType = (projectType === "STARTER"
    ? "VITRINE"
  ...
  const mandatoryModuleIds = useMemo(
    () => (projectType === "STARTER" ? [] : getMandatoryModules(offerProjectType, offerStack)),
    [offerProjectType, offerStack, projectType],
  );

  const includedModuleIds = useMemo(
    () => (projectType === "STARTER" ? [] : getIncludedModules(offerProjectType, offerStack)),
    [offerProjectType, offerStack, projectType],
  );
  ...
  const totalDisplay = estimate
    ? (estimate.totalSetup.min === estimate.totalSetup.max
      ? formatEur(estimate.totalSetup.min)
      : `${formatEur(estimate.totalSetup.min)} à ${formatEur(estimate.totalSetup.max)}`)
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <form action={formAction} className="space-y-6">
        ...
        <input type="hidden" name="catSelections" value={JSON.stringify(catSelections)} />
        <input type="hidden" name="maintenanceLevel" value={qualification.maintenance} />
        <input type="hidden" name="estimatedBudget" value={manualEstimatedBudget} />
            ...
            <div className="space-y-2">
              <FieldSelect
                ...
                onChange={(value) => {
                  ...
                  if (
                    techStack &&
                    !getAllowedTechStacksForProjectType(newType).includes(techStack as TechStack)
                  ) {
                    setTechStack("");
                    setWpHeadless(false);
                  }
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              {TECH_STACK_OPTIONS
                .filter((stack) =>
                  getAllowedTechStacksForProjectType(projectType as ProjectType).includes(stack.value)
                )
                .map((stack) => {
          ...
          <CardContent>
            ...
            <div className="mt-4 space-y-2">
              <FieldSelect
                ...
                onChange={(value) => {
                  setHostingProviderId(value as HostingProviderId);
                }}
                options={providers.map((provider) => ({
                  value: provider.id,
                  label:
                    provider.id === defaultHostingProvider
                      ? `${provider.label} - recommandé`
                      : provider.label,
                }))}
              />
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2 rounded-lg border p-3">
              <div className="flex items-center justify-between">
                <Label htmlFor="manual-estimated-budget">Tarif manuel (HT)</Label>
                {manualEstimatedBudget.trim() && (
                  <Button
                    type="button"
                    variant="ghost"
                    size="sm"
                    onClick={() => setManualEstimatedBudget("")}
                  >
              ...
              <Input
                id="manual-estimated-budget"
                type="number"
                min={0}
                step={100}
                value={manualEstimatedBudget}
                onChange={(event) => setManualEstimatedBudget(event.target.value)}
                placeholder="Ex: 4200"
              />
              <p className="text-xs text-muted-foreground">
                Optionnel. Si renseigné, ce montant est enregistré comme budget estimé du projet.
              </p>
              <p
                className={`text-xs ${
                  manualEstimatedBudget.trim()
              ...
              >
                {manualEstimatedBudget.trim() ? "Mode manuel actif." : "Mode auto actif."}
              </p>
            </div>
                    ...
                    <div className="grid gap-2 sm:grid-cols-2">
                      {visibleMods.map((mod) => (
                        <ModuleCard
                          ...
                          onTierChange={(modId, ts) => { setCatSelections((prev) => ({ ...prev, [modId]: ts })); }}
                          techStack={resolvedTechStack}
                          projectType={projectType as ProjectType}
