  ...
  return (
    <div className="rounded-lg border p-4 space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Liste complète des events planifiés.
          </p>
        </div>
        <Button
          size="sm"
      ...
      <div className="text-xs text-muted-foreground">
        <p>Events détectés: {cronEvents ? String(cronEvents.length) : "n/a"}</p>
        <p>Prochain: {formatDate(nextCron)}</p>
      </div>
      {sortedCronEvents.length === 0 ? (
        <p className="text-xs text-muted-foreground">Aucun event détecté.</p>
      ) : (
