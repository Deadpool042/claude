...
export function TabCommercial({ project, config }: TabCommercialProps) {
  ...
  const maintenance = config?.maintenanceLevel as MaintenanceCat | null;
  const budget = config?.estimatedBudget;

  const toSpecCategory = (category: string | null): SpecCategory | null => {
  ...
  const estimate = (() => {
    const category = toSpecCategory(project.category);
    if (!category) return null;
    try {
  ...
  const groupedModules = MODULE_CATALOG.reduce<Record<string, ModuleDef[]>>((acc, mod) => {
    if (!acc[mod.group]) {
      acc[mod.group] = [];
    }
  ...
  // ── IA subscription ──
  const automationModule = activeModules.find((m) => m.id === "module.MARKETING_AUTOMATION");
  const hasIa = automationModule != null;
  // Default subscription = lowest available tier (if present)
  const iaEstimated = automationModule ? estimateModuleById.get(automationModule.id) : null;
  const iaMonthly = iaEstimated?.monthly?.min ?? 0;
  // All subscription tiers for display
  const iaSubscriptionCats = automationModule?.subscriptionCats ?? [];

  // ── Coûts récurrents totaux ──
  const totalMonthly =
    estimate?.modules.reduce((sum, moduleItem) => sum + (moduleItem.monthly?.min ?? 0), 0) ?? 0;
                    ...
                    <div key={groupKey} className="space-y-2">
                      ...
                      <div className="grid gap-2 sm:grid-cols-2">
                        {groupActive.map((mod) => {
                          ...
                          const moduleEstimate = estimateModuleById.get(mod.id);
      ...
      {hasIa && (
        <Card className="border-blue-200 dark:border-blue-800">
          ...
          <CardContent>
            ...
            {iaSubscriptionCats.length > 0 && (
              <div className="mt-3 space-y-1.5">
                <p className="text-xs font-medium text-muted-foreground">
                  Paliers disponibles :
                </p>
                {iaSubscriptionCats.map((tier) => (
                  <div
                    key={tier.id}
